...
export const api = {
    create: async (params) => {
        const { data } = await axios.post(`return-order`, params)
        return data?.data
    },
    update: async (id, params) => {
        const { data } = await axios.put(`return-order/${id}`, params)
        return data?.data
    },
    status: async (id, params) => {
        const { data } = await axios.post(`return-order/${id}/status`, params)
        return data?.data
    },
    submitWithStatus: async (id, params) => {
        const { data } = await axios.post(`return-order-with-status/${id}`, params)
        return data?.data
    },
    submitMigrate: async (id, params) => {
        // console.log('Sending migration request with:', params)
        const { data } = await axios.post(`return-order-migrate/${id}`, params)
        // console.log('Received migration response:', data)
        return data
    },
    submitMigrateAll: async (id, params) => {
        // console.log('Sending migration request with:', params)
        const { data } = await axios.post(`return-orders-migrate`)
        // console.log('Received migration response:', data)
        return data
    },
    supplier: async (id, params) => {
        const { data } = await axios.post(`return-order/${id}/supplier`, params)
        return data?.data
    },
    shippingStatus: async (id, { status: shipping_status }) => {
        const { data } = await axios.post(`return-order/${id}/shipping-status`, { shipping_status })
        return data?.data
    }
}
...
export function useOrder(id) {
    ...
    const url = `return-order/${id}`
    const url2 = `return-order-migrate/${id}`

    const { data, error } = useSWR(`return-order/${id}`, fetcher, {
...
export function useDatatable({ page, limit, search, order = {}, conditions = {} }) {
    ...
    const url = `return-order/datatable?page=${page}&limit=${limit}&search=${search}&order=${JSON.stringify(order)}&conditions=${JSON.stringify(conditions)}`
