...
import ReceiptWithoutHeaderInvoice from '@components/invoice/receipt'
...
const Print = () => {
    ...
    useEffect(() => {
        if (order && !printTriggered.current) {
            ...
            const customerName = order.customer?.name || 'Receipt'
    ...
    return (
        <ReceiptWithoutHeaderInvoice order={order} meta={calcFinancial(order)} onPageChange={setCurrentPage} />
    )
}
