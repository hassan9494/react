...
export default function Tafgeet(digit) {
...
Tafgeet.prototype.parse = function () {
    ...
    var concats = []
    for (let i = this.getColumnIndex(); i < this.columns.length; i++) {
        concats[i] = " و";
    }

    //We do not needs some "و"s check last column if 000 drill down until otherwise
    if (this.digit > 999) {
...
Tafgeet.prototype.currencies = {
    ...
    JOD: {
        singular: "دينارًا أردنيًا",
        plural: "دينارات أردنية",
        fraction: "قرش",
        fractions: "قروش",
        decimals: 2
    },
